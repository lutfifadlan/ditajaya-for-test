    ...
    <x-admin::form.control-group class="mb-2.5 w-full">
        ...
        @if (isset($attribute))
            @if ($attribute->code == 'user_id')
                <x-admin::attributes.edit.index :attribute="$attribute" :validations="$validations" :value="isset($entity) ? $entity[$attribute->code] : auth()->id()" />
            @else
                <x-admin::attributes.edit.index :attribute="$attribute" :validations="$validations" :value="isset($entity) ? $entity[$attribute->code] : null" />
            @endif
        @endif
