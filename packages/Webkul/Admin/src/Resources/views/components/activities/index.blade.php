@props(['endpoint', 'emailDetachEndpoint' => null, 'activeType' => 'all', 'types' => null, 'extraTypes' => null])

{!! view_render_event('admin.components.activities.before') !!}

<!-- Lead Activities Vue Component -->
<v-activities endpoint="{{ $endpoint }}" email-detach-endpoint="{{ $emailDetachEndpoint }}"
    active-type="{{ $activeType }}" @if ($types) :types='@json($types)' @endif
                ...
                markAsDone: function(activity) {
                    ...
                    this.$emitter.emit('open-confirm-modal', {
                        agree: () => {
                            ...
                            this.$axios.put("{{ route('admin.activities.update', 'replaceId') }}"
                                    .replace('replaceId', activity.id), {
                                        'is_done': 1
                                    })
                ...
                remove: function(activity) {
                    ...
                    this.$emitter.emit('open-confirm-modal', {
                        agree: () => {
                            ...
                            this.$axios.delete(
                                    "{{ route('admin.activities.delete', 'replaceId') }}".replace(
                                        'replaceId', activity.id))
                                .then(function(response) {
                                    self.isUpdating[activity.id] = false;

                                    self.activities.splice(self.activities.indexOf(activity),
                                    1);
                    ...
                    this.$emitter.emit('open-confirm-modal', {
                        agree: () => {
                            ...
                            this.$axios.delete(this.emailDetachEndpoint, {
                                ...
                                .then(response => {
                                    let relatedActivities = self.activities.filter(activity =>
                                        activity.parent_id == emailId || activity.id ==
                                        emailId);

                                    relatedActivities.forEach(activity => {
                                        const index = self.activities.findIndex(a =>
                                            a === activity);
