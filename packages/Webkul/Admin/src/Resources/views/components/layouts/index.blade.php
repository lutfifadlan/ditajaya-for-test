...
<body class="h-full font-inter dark:bg-gray-950">
    ...
    <div id="app" class="h-full">
        ...
        <div class="group/container sidebar-collapsed flex gap-4" ref="appLayout">
            ...
            <div
                class="max-w-full flex-1 bg-gray-100 px-4 pb-6 pt-3 transition-all duration-300 dark:bg-gray-950 max-lg:!px-4 ltr:pl-[85px] rtl:pr-[85px]">
    ...
    @if (str_contains(url()->current(), 'technician'))
    ...
    <script>
        ...
        window.addEventListener("load", function(event) {
            app.mount("#app");
            if (window.location.pathname == "/admin/technician/create") {
                $('.select2').select2();
                var sig = $('#sig').signature({
                    syncField: '#svgSig',
                    syncFormat: 'PNG'
                });
                var customer_sig = $('#customer_sig').signature({
                    syncField: '#svgSigCustomer',
                    syncFormat: 'PNG'
                });
            }
